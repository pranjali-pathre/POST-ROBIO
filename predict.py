...
def test(args):
    ...
    with torch.no_grad():
        for inputs in tqdm(val_loader):
            ...
            #         for sem_idx, sem_class in enumerate(["unknown", "occupied"]):
            ...
            for idx in range(preds.shape[0]):
                ...
                folder = inputs["filename"][idx].split()[0].replace("/scratch/POST-ROBIO/data/img/", "")
                print(folder)
                output_path = os.path.join(args.out_dir, model_name, folder)
                ...
                if (("rgb_cam", "unknown", 0) in outputs):
                    for sem_class in ["unknown", "occupied"]:
                        bev_dir = 'grad_rgb'
